...
compute_D_ortho_components(const line_t &line, const Vector3 &W, Vector3 &Dw,
...
__host__ __device__ void compute_D_ortho(line_t &line, const Vector3 &W) {
  Vector3 Dw, D_ortho;
  real_t norm_Dw;
  compute_D_ortho_components(line, W, Dw, norm_Dw, D_ortho);
...
__host__ __device__ void update_derivatives(line_t &line,
                                            const Vector3 &sensor_direction) {
  ...
  Vector3 Dw, D_ortho;
  real_t norm_Dw;
  compute_D_ortho_components(line, sensor_direction, Dw, norm_Dw, D_ortho);
  ...
  line.dD_ortho[THETA] =
      get_delta_D_ortho(line, THETA, D_ortho, norm_Dw, sensor_direction);
  line.dD_ortho[PHI] =
      get_delta_D_ortho(line, PHI, D_ortho, norm_Dw, sensor_direction);

  // Compute second derivatives of D_ortho
  line.ddD_ortho[DD_THETA_THETA] =
      get_dd_D_ortho(line, THETA, THETA, norm_Dw, sensor_direction);
  line.ddD_ortho[DD_PHI_PHI] =
      get_dd_D_ortho(line, PHI, PHI, norm_Dw, sensor_direction);
  line.ddD_ortho[DD_THETA_PHI] =
      get_dd_D_ortho(line, THETA, PHI, norm_Dw, sensor_direction);
}
} // namespace lineMath