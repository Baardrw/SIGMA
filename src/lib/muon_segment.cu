...
__device__ inline void initialize_bucket_indexing(struct Data *data, 
                                                  int bucket_index,
                                                  int &bucket_start, 
                                                  int &rpc_start, 
                                                  int &bucket_end,
...
__device__ void seed_line_impl(struct Data *data, const int thread_data_index,
    ...
    for (int j = 0; j < 4; j++) {
        ...
        residualMath::compute_residual(line, K, W, drift_radius, 
                                      bucket_tile.thread_rank(),
                                      num_mdt_measurements, num_rpc_measurements, 
                                      residual_cache);

        chi2_arr[j] = residualMath::get_chi2<TILE_SIZE>(bucket_tile, 
...
__device__ void fit_line_impl(struct Data *data, const int thread_data_index,
    ...
    for (int iteration = 0; iteration < 1000; iteration++) {
        ...
        lineMath::create_line(params[X0], params[Y0], params[PHI], params[THETA], line);
        ...
        residualMath::compute_delta_residuals(line, bucket_tile.thread_rank(), 
                                             num_mdt_measurements, num_rpc_measurements, 
                                             K, W, residual_cache);

        residualMath::compute_dd_residuals(line, bucket_tile.thread_rank(), 
                                          num_mdt_measurements, num_rpc_measurements, 
                                          K, W, residual_cache);

        // Get gradient and hessian
        Vector4 gradient = residualMath::get_gradient<TILE_SIZE>(bucket_tile, 
                                                                inverse_sigma_squared, 
                                                                residual_cache);
        Matrix4 hessian = residualMath::get_hessian<TILE_SIZE>(bucket_tile, 
                                                              inverse_sigma_squared, 
                                                              residual_cache);

        // Newton step (thread 0 only, using 2x2 system because im too lazy to instantly do
        if (bucket_tile.thread_rank() == 0) {
...
__device__ void partition_and_execute_work(struct Data *data, int num_buckets,
    ...
    const int secondary_bucket = (threadIdx.x < MAX_MPB) ? primary_bucket + 1 : primary_bucket - 1;
    ...
    for (int i = 0; i < 2; i++) {
        if (bucket_indices[i] < num_buckets) {
            initialize_bucket_indexing<MAX_MPB>(data, bucket_indices[i],
                                               bucket_starts[i], rpc_starts[i],
    ...
    const bool has_overflow = (bucket_starts[0] != -1 && (bucket_ends[0] - bucket_starts[0]) > MAX_MPB) ||
    ...
    if (!has_overflow) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            initialize_bucket_indexing<OVERFLOW_TILE_SIZE>(data, current_bucket,
                                                          bucket_start, rpc_start,
                                                          bucket_end, thread_data_index);
