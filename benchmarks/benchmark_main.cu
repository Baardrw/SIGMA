...
#define NUM_BUCKETS 10000 // Number of desired bins will be determined by this
...
void create_round_robin_data(Data &h_data, 
                           const std::vector<long> &original_bucket_ids,
    ...
    std::cout << "Creating " << desired_buckets << " buckets from " << original_buckets 
    ...
    for (int bucket_idx = 0; bucket_idx < desired_buckets; bucket_idx++) {
        uint64_t original_bucket_id = original_bucket_ids[bucket_idx % original_buckets];
        ...
        h_data.buckets[bucket_idx * 3 + 1] = running_total + mdt_hits.size();  // rpc_start
        h_data.buckets[bucket_idx * 3 + 2] = running_total + mdt_hits.size() + rpc_hits.size(); // bucket_end
        ...
        for (const auto &hit : mdt_hits) {
            ...
            // Calculate drift radius as distance between tube position and hit position
        ...
        for (const auto &hit : rpc_hits) {
            ...
            h_data.drift_radius[measurement_idx] = 0.0f; // RPC doesn't have drift radius
            h_data.sigma[measurement_idx] = 0.1f;
            measurement_idx++;
        }
        ...
        if (bucket_idx < 10 || bucket_idx % (desired_buckets / 10) == 0) {
            std::cout << "Bucket " << bucket_idx << " (from original bucket " 
                      ...
                      << (original_bucket_id >> 32) << "): "
                      << mdt_hits.size() << " MDT + " << rpc_hits.size() 
                      << " RPC hits" << std::endl;
        }
    }
...
bool benchmark_seed_line_with_round_robin_data() {
  ...
  auto [bucket_ids, mdt_bucket, rpc_bucket, bucket_ground_truths] =
      load_and_organize_data(mdt_filename, rpc_filename, start_event, end_event);
  ...
  int total_measurements = h_data.buckets[(desired_buckets - 1) * 3 + 2]; // End of last bucket
  std::cout << "Total measurements to process: " << total_measurements << std::endl;
  ...
  std::cout << "Duplication factor: " << (float)desired_buckets / original_buckets << std::endl;
  ...
  const int num_blocks = desired_buckets / (warps_per_block * (warpSize / cg_group_size)) +
      (desired_buckets % (warps_per_block * (warpSize / cg_group_size)) == 0 ? 0 : 1);
  ...
  auto load_time = std::chrono::duration<double, std::milli>(end_load - start_load).count();
  auto setup_time = std::chrono::duration<double, std::milli>(end_setup - start_setup).count();
  auto h2d_time = std::chrono::duration<double, std::milli>(end_h2d - start_h2d).count();
  auto d2h_time = std::chrono::duration<double, std::milli>(end_d2h - start_d2h).count();
  auto total_time = std::chrono::duration<double, std::milli>(end_total - start_total).count();
  auto kernel_total_time = std::chrono::duration<double, std::milli>(
                               end_kernel_total - start_kernel_total).count();

  // Kernel timing statistics
  float min_kernel_time = *std::min_element(kernel_times.begin(), kernel_times.end());
  float max_kernel_time = *std::max_element(kernel_times.begin(), kernel_times.end());
  float avg_kernel_time = std::accumulate(kernel_times.begin(), kernel_times.end(), 0.0f) /
                         kernel_times.size();
  ...
  // Performance metrics
  double measurements_per_second = (total_measurements * num_runs) / (kernel_total_time / 1000.0);
  double buckets_per_second = (desired_buckets * num_runs) / (kernel_total_time / 1000.0);

  // Memory bandwidth estimation (rough)
  size_t estimated_bytes_transferred =
      total_measurements * (sizeof(float) * 6 + sizeof(int)) + // Input data per measurement
  ...
  std::cout << "\n=== BENCHMARK RESULTS (Round-Robin Duplication) ===" << std::endl;
  ...
  std::cout << "Duplication factor:          " << (float)desired_buckets / original_buckets << "x" << std::endl;
  std::cout << "Total measurements:          " << total_measurements << std::endl;

  std::cout << "\n--- Timing Breakdown ---" << std::endl;
  std::cout << "Data loading time:           " << load_time << " ms" << std::endl;
  std::cout << "Round-robin setup time:      " << setup_time << " ms" << std::endl;
  std::cout << "Host to Device copy:         " << h2d_time << " ms" << std::endl;
  std::cout << "Device to Host copy:         " << d2h_time << " ms" << std::endl;
  std::cout << "Total time:                  " << total_time << " ms" << std::endl;

  std::cout << "\n--- Kernel Performance (average of " << num_runs << " runs) ---" << std::endl;
  std::cout << "Average kernel time:         " << avg_kernel_time << " ms" << std::endl;
  std::cout << "Min kernel time:             " << min_kernel_time << " ms" << std::endl;
  std::cout << "Max kernel time:             " << max_kernel_time << " ms" << std::endl;
  std::cout << "Standard deviation:          " << std_dev << " ms" << std::endl;
  std::cout << "Coefficient of variation:    " << (std_dev / avg_kernel_time * 100) << "%" << std::endl;

  std::cout << "\n--- Performance Metrics ---" << std::endl;
  std::cout << "Measurements per second:     " << std::scientific << measurements_per_second << std::endl;
  std::cout << "Buckets per second:          " << buckets_per_second << std::endl;
  std::cout << "Est. memory bandwidth:       " << std::fixed << memory_bandwidth_gb_s << " GB/s" << std::endl;
...
int main() {
  std::cout << "Starting GPU kernel benchmark with round-robin CSV data..." << std::endl;
